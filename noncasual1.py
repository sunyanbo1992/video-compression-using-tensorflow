...
random = np.random.randint(255, size = [64, 64])
...
for img in target_image:
    ...
    for i in xrange(6):
        ...
        for j in xrange(6):
            ...
            for x in xrange(3):
                all_rows_concatenated.append(np.concatenate([blocks_training[i + x, j], blocks_training[i + x, j + 1], blocks_training[i + x, j + 2]], axis = 1))
...
for img in test_image:
    ...
    for i in xrange(6):
        for j in xrange(6):
            all_rows_concatenated = []
            for x in xrange(3):
                all_rows_concatenated.append(np.concatenate([blocks_test[i + x, j], blocks_test[i + x, j + 1], blocks_test[i + x, j + 2]], axis = 1))
            combined_block_training = np.concatenate(all_rows_concatenated, axis = 0)
            batch.append(combined_block_training)
...
X = tf.placeholder("float", [None, 576], name='X')
Z_ = tf.placeholder("float", [None, 64], name = "Z")
keep_prob = tf.placeholder("float")

initializer = tf.contrib.layers.xavier_initializer()

W = tf.Variable(initializer([576, n_hidden]))
...
def model(X, W, b):
    ...
    Y_1 = tf.nn.relu(tf.matmul(Y, W_2) + b_2)
    
    Y_2 = tf.nn.relu(tf.matmul(Y_1, W_3) + b_3)

    Y_3 = tf.nn.tanh(tf.matmul(Y_2, W_4) + b_4)
...
train_op = tf.train.RMSPropOptimizer(0.01).minimize(cost)  # construct an optimizer
...
with tf.Session() as sess:
    ...
    for i in range(100):
        ...
        for j in range(len(image_training)):
            ...
            batch = batch.reshape(-1, 576)
            batch_1 = batch_1.reshape(-1, 64)

            sess.run(train_op, feed_dict={X: batch, Z_: batch_1, keep_prob: 0.5})
    ...
    for j in xrange(len(image_test)):
        for n in xrange(len(image_test[j])):
            ...
            reconstruct_matrix = quant_matrix.reshape(-1, 576)
